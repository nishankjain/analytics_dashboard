var adwyzeAppDirectives = angular.module('adwyzeDirectives', []);

adwyzeAppDirectives.directive('changeChart', function() {
	return {
		restrict: 'A',
		scope: true,
		link: function(scope, element, attrs) {
			element
			.on('click', function() {
				scope.divChartType = scope.chartType;
				...
				if (currentlyActiveElement) {
					...
					if (angularElement.scope().activeChart) {
						...
						var selectedChartType = scope.divChartType;
						eval(selectedChartType + '(currentlyActiveElement, dataArray)');
						// splitHorizontally(currentlyActiveElement);
					}
				}
			})
		}
...
adwyzeAppDirectives.directive('getType', function() {
	return {
		restrict: 'A',
		scope: true,
		link: function(scope, element, attrs) {
			// console.log(typeof scope.$eval(attrs.getType));
			scope.dataType = typeof scope.$eval(attrs.getType);
			console.log(scope);
		}
	};
});

adwyzeAppDirectives.directive('cardsDiv', function() {
	return {
		restrict: 'A',
		scope: true,
		link: function(scope, element, attrs) {
...
adwyzeAppDirectives.directive('splitHori', function($compile) {
	return {
		restrict: 'A',
		link: function(scope, element, attrs) {
			...
			var splitHorizontally = function(currentlyActiveElement) {
...
adwyzeAppDirectives.directive('splitVert', function($compile) {
	return {
		restrict: 'A',
		link: function(scope, element, attrs) {
			element.on('click', function() {
				...
				if (currentlyActiveElement) {
					var angularElement  = angular.element(currentlyActiveElement);
					if (angularElement.scope().activeChart) {
						splitVertically(currentlyActiveElement);
					}
				}
			});

			var splitVertically = function(currentlyActiveElement) {
				var active_top = parseFloat(currentlyActiveElement.style.top);
				var active_right = parseFloat(currentlyActiveElement.style.right);
				var active_bottom = parseFloat(currentlyActiveElement.style.bottom);
				var active_left = parseFloat(currentlyActiveElement.style.left);
